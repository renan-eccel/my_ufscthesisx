...
\ifenglish
	% The abntex2 package, hard codes the language portuguese. Therefore we are required to edit the
	% package `abntex2`, usually on:
	% `D:\User\Documents\latex\texmfs\install\tex\generic\babel\babel.sty` and change the line
	% `\RequirePackage[brazil]{babel}` to `\RequirePackage[english]{babel}`.
	%
...
% ---
% Configurações do pacote backref, paginas com as citações na bibl
